<div align="center">

# 👋 Hi, I'm Shawon Talukder

### Full-Stack Software Engineer · Angular Specialist · Mobile Developer

**Building modern web, backend & mobile applications with a focus on clean architecture,
practical solutions, and great user experiences.**

<br/>

[![GitHub](https://img.shields.io/badge/GitHub-Melodious--nub-181717?style=for-the-badge\&logo=github)](https://github.com/Melodious-nub)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge\&logo=linkedin)](https://www.linkedin.com/in/shawon-taluckder)
[![Fiverr](https://img.shields.io/badge/Fiverr-Hire%20Me-1DBF73?style=for-the-badge\&logo=fiverr\&logoColor=white)](https://www.fiverr.com/talukder_shawon/)
[![Email](https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge\&logo=gmail\&logoColor=white)](mailto:shawon.taluckder2@gmail.com)

</div>

---

## 👨‍💻 About Me

I'm a **Full-Stack Software Engineer with 5+ years of professional experience**, specializing in modern web application development and enterprise software.

My strongest area is **Angular & TypeScript**, complemented by hands-on experience in backend development, databases, mobile applications, API integration, and production deployment.

I enjoy taking ideas from **concept → architecture → implementation → production**, while continuously exploring better tools and technologies along the way.

---

## ⚡ What I Do

| Area            | Technologies                                                       |
| --------------- | ------------------------------------------------------------------ |
| **Frontend**    | Angular · TypeScript · JavaScript · React · Vue · Next.js · Nuxt   |
| **Backend**     | Node.js · Express.js · NestJS · REST APIs                          |
| **Mobile**      | Kotlin · Android · Jetpack Compose · iOS · Capacitor               |
| **Database**    | MySQL · MongoDB · PostgreSQL                                       |
| **Engineering** | Architecture · API Integration · Reusable Components · Performance |
| **Tools**       | Git · GitHub · Linux · Postman · Swagger/OpenAPI · cPanel · VPS    |

---

## 🤖 AI-Assisted Development

Modern AI tools are part of my development workflow for **research, problem solving, code exploration, refactoring, documentation, prototyping, and productivity**.

<p align="left">

![Claude](https://img.shields.io/badge/Claude-D97757?style=flat-square\&logo=anthropic\&logoColor=white)
![OpenAI Codex](https://img.shields.io/badge/OpenAI%20Codex-412991?style=flat-square\&logo=openai\&logoColor=white)
![Google Gemini](https://img.shields.io/badge/Gemini-8E75B2?style=flat-square\&logo=google\&logoColor=white)
![Antigravity](https://img.shields.io/badge/Antigravity-111111?style=flat-square)

</p>

> AI accelerates the workflow — engineering decisions, architecture, validation, and responsibility remain developer-driven.

---

# 🚀 Selected Projects

## BMCCI — Bangladesh Malaysia Chamber of Commerce & Industry

A modern digital platform developed for the **Bangladesh Malaysia Chamber of Commerce & Industry**, focused on delivering a contemporary and engaging web experience.

**Focus:** Modern Web · Responsive Design · User Experience · Performance

🔗 **[Live Website](https://bmcci-kappa.vercel.app/)**
🔗 **[Source Code](https://github.com/Melodious-nub/bmcci)**

---

## GDAMS — Green Delta Asset Management System

An enterprise asset management platform developed for **Green Delta Insurance PLC**, supporting asset tracking and operational management workflows.

**Focus:** Enterprise Software · Angular · API Integration · Business Applications

🔗 **[Project](https://gdams.green-delta.com/)**

---

## Com-Box — Grievance Management Platform

A multilingual grievance and feedback management platform designed for RMG organizations, supporting anonymous feedback across **30+ languages**.

The platform has been adopted by organizations and suppliers associated with international brands including **Vaude, Jack Wolfskin, Madness, and Marc Cain**.

**Focus:** Enterprise Software · Multilingual Systems · Angular · API Integration

---

## Vote-BNP

A web platform developed for digital engagement and political campaign activities.

**Focus:** Web Application · Responsive UI · User Experience

🔗 **[Live Website](https://vote-bnp.com/)**

---

## Olympiad App

A registration and management platform designed to streamline the Olympiad workflow, from participant registration to result management and publication.

Developed as a **charitable initiative**.

**Focus:** Registration · Management System · Web Application

---

## HRM — Human Resource Management System

A human resource management solution designed to support employee information, attendance, payroll, and reporting workflows.

**Focus:** Business Software · HR Management · Reporting

---

# 📱 Open Source & Released Apps

## 🔒 ScreenLocker

A lightweight and privacy-focused Android application that lets users **lock their device screen with a single tap**.

Built to solve a simple real-world problem while keeping the application minimal, offline, and privacy-focused.

### Highlights

* 📱 Native Android application
* 🟣 Built with Kotlin
* 🔐 Android Accessibility API
* 🌐 No internet access
* 🚫 No ads or tracking
* 🛡️ Privacy-focused by design
* 📲 Android 9.0+ support

<p>
<a href="https://github.com/Melodious-nub/screenlocker">
<img src="https://img.shields.io/badge/View%20Source%20Code-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>
<a href="https://github.com/Melodious-nub/screenlocker/releases/latest">
<img src="https://img.shields.io/badge/Download%20Latest%20Release-2EA44F?style=for-the-badge&logo=android&logoColor=white" />
</a>
</p>

---

# 🧩 Development Philosophy

```text
Understand the Problem
        ↓
Design the Solution
        ↓
Build with Clean Architecture
        ↓
Test & Refine
        ↓
Ship to Production
        ↓
Keep Improving
```

I believe good software is not just about making things work —
it's about making them **maintainable, reliable, performant, and useful**.

---

# 🤝 Let's Connect

I'm always interested in **interesting ideas, technical discussions, collaborations, freelance opportunities, and opportunities to build useful software.**

<div align="center">

📧 **[shawon.taluckder2@gmail.com](mailto:shawon.taluckder2@gmail.com)**

📱 **+880 1772-219732**

💼 **[LinkedIn Profile](https://www.linkedin.com/in/shawon-taluckder)**

🟢 **[Fiverr Profile](https://www.fiverr.com/talukder_shawon/)**

🐙 **[GitHub Profile](https://github.com/Melodious-nub)**

<br/>

### ⚡ Build · Ship · Learn · Repeat

<sub>Turning ideas into software, one project at a time.</sub>

</div>
